...
#pragma omp parallel for simd aligned(u11t:AVX) 
		...
		hipMemPrefetchAsync(u12t, ndim*kvol*sizeof(Complex),device,streams[1]);
#endif
		...
		hipMemPrefetchAsync(u12, ndim*kvol*sizeof(Complex),device,streams[1]);
#else
...
int Hamilton(double *h, double *s, double res2, double *pp, Complex *X0, Complex *X1, Complex *Phi,\
	...
	int device=-1;
	hipGetDevice(&device);
	hipMemPrefetchAsync(pp,kmom*sizeof(double),device,NULL);
	hipblasDnrm2(cublas_handle, kmom, pp, 1,&hp);
	hp*=hp;
...
#endif
		hipMemcpyAsync(X1,X0+na*kferm2,kferm2*sizeof(Complex),hipMemcpyDeviceToDevice,streams[0]);
#ifdef _DEBUG
		cudaDeviceSynchronise();
#endif
#else
