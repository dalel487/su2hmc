...
int Gauge_force(double *dSdpi, Complex_f *u11t, Complex_f *u12t,unsigned int *iu,unsigned int *id, float beta){
	...
	hipMallocAsync((void **)&Sigma11,kvol*sizeof(Complex_f),streams[0]);
	hipMallocAsync((void **)&Sigma12,kvol*sizeof(Complex_f),streams[1]);
	hipMallocManaged((void **)&u11sh,(kvol+halo)*sizeof(Complex_f),hipMemAttachGlobal);
	hipMallocManaged((void **)&u12sh,(kvol+halo)*sizeof(Complex_f),hipMemAttachGlobal);
#else
...
#pragma unroll
	...
	hipMallocAsync((void **)&X2_f,kferm2*sizeof(Complex_f),NULL);
...
//	Transpose_z(X1,kvol,ndirac*nc,dimGrid,dimBlock); Transpose_z(X2,kvol,ndirac*nc,dimGrid,dimBlock);
	...
	cudaDeviceSynchronise();
	hipFreeAsync(X1_f,NULL); hipFreeAsync(X2_f,NULL);
#else
