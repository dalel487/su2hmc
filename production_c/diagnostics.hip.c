...
#include<float.h>
	...
	hipMallocManaged(&R1_f,kfermHalo*sizeof(Complex_f),hipMemAttachGlobal);
	hipMallocManaged(&xi_f,kfermHalo*sizeof(Complex_f),hipMemAttachGlobal);
	hipMallocManaged(&Phi,kfermHalo*sizeof(Complex),hipMemAttachGlobal);
	hipMallocManaged(&Phi_f,kfermHalo*sizeof(Complex_f),hipMemAttachGlobal);
	hipMallocManaged(&X0,kferm2Halo*sizeof(Complex),hipMemAttachGlobal);
	hipMallocManaged(&X1,kferm2Halo*sizeof(Complex),hipMemAttachGlobal);
	hipMallocManaged(&X0_f,kferm2Halo*sizeof(Complex_f),hipMemAttachGlobal);
	hipMallocManaged(&X1_f,kfermHalo*sizeof(Complex_f),hipMemAttachGlobal);
	hipMallocManaged(&pp,kmomHalo*sizeof(double),hipMemAttachGlobal);
	hipMallocManaged(&dSdpi,kmom*sizeof(double),hipMemAttachGlobal);
#else
...
#ifdef __NVCC__
				hipMemPrefetchAsync(R1,kferm*sizeof(Complex),device,NULL);
				hipMemPrefetchAsync(xi,kferm*sizeof(Complex),device,NULL);
				hipMemPrefetchAsync(R1_f,kferm*sizeof(Complex_f),device,NULL);
				hipMemPrefetchAsync(xi_f,kferm*sizeof(Complex_f),device,NULL);
...
#pragma omp parallel for simd
...
#ifdef __NVCC__
				hipMemPrefetchAsync(X0,kferm2*sizeof(Complex),device,NULL);
				hipMemPrefetchAsync(X1,kferm2*sizeof(Complex),device,NULL);
				hipMemPrefetchAsync(X0_f,kferm2*sizeof(Complex_f),device,NULL);
			...
			case(7):
				...
				fclose(output_old);	
#ifdef __NVCC__
				hipMemPrefetchAsync(dSdpi,kmom*sizeof(double),device,NULL);
#endif
	...
	hipFree(X0_f); hipFree(X1_f); hipFree(u11t_f); hipFree(u12t_f);
