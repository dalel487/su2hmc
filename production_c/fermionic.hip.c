...
int Measure(double *pbp, double *endenf, double *denf, Complex *qq, Complex *qbqb, double res, int *itercg,\
	...
	#ifdef _DEBUG
	hipMallocManaged((void **)&R1_f,kfermHalo*sizeof(Complex_f), hipMemAttachGlobal);
	#else
	hipMallocAsync((void **)&R1_f,kfermHalo*sizeof(Complex_f),streams[1]);
	#endif
	hipMallocManaged((void **)&x,kfermHalo*sizeof(Complex), hipMemAttachGlobal);
	hipMallocManaged((void **)&xi,kferm*sizeof(Complex), hipMemAttachGlobal);
	hipMallocManaged((void **)&xi_f,kfermHalo*sizeof(Complex_f), hipMemAttachGlobal);
#else
	...
	hipMemcpyAsync(x, xi, kferm*sizeof(Complex),hipMemcpyDefault,0);
#else
#pragma omp parallel for simd aligned(xi,xi_f:AVX)
	...
	hipMemcpy(Phi, R1, kferm*sizeof(Complex),hipMemcpyDefault);
#else
