...
C     INCLUDE COMMON BLOCK DEFINITIONS - HEADER FILE FOR EACH LABEL 
      ...
      REAL(KIND=REALKIND) VEL2A,PBPA,ENDENFA,DENFA,YAV,YYAV,PBP
      COMMON/VECTORP/ XI(NC,NGORKOV,KVOL+HALO)
...
18    CONTINUE
      ENDIF
C!     $OMP PARALLEL SECTIONS NUM_THREADS(2)
C!     $OMP SECTION
C      DO 14 I = 1, KVOL+HALO
C      WRITE(7000,16) REAL(U11T(I,1)),AIMAG(U11T(I,1)),
C     &REAL(U11T(I,2)),AIMAG(U11T(I,2)), 
C     &REAL(U11T(I,3)),AIMAG(U11T(I,3)), 
C     &REAL(U11T(I,4)),AIMAG(U11T(I,4)) 
C14    CONTINUE
C!     $OMP SECTION
C      DO 15 I = 1, KVOL+HALO
C      WRITE(8000,16) REAL(U12T(I,1)),AIMAG(U12T(I,1)),
C     &REAL(U12T(I,2)),AIMAG(U12T(I,2)), 
C     &REAL(U12T(I,3)),AIMAG(U12T(I,3)), 
C     &REAL(U12T(I,4)),AIMAG(U12T(I,4)) 
C15    CONTINUE
C!     $OMP END SECTIONS
C16    FORMAT(F9.6,'+I',1X,F9.6,6X,F9.6,'+I',1X,F9.6,6X
C     &       F9.6,'+I',1X,F9.6,6X,F9.6,'+I',1X,F9.6)
      DO 2 TEST = 1000, 7000, 1000
!     $OMP PARALLEL FOR
      DO 3 I = 1, KVOL
!     $OMP SIMD COLLAPSE(2)      
...
21    CONTINUE
      ENDIF
c	SWAPPING TEST ORDER TO AVOID POTENTIAL HICCUPS
      ...
      ELSEIF(TEST.EQ.7000) THEN
      CALL MEASURE(PBP,ENDENF,DENF,QQ,QBQB,RESPBP,ITERCG)
      WRITE(TEST+1,*) 'PBP',PBP,'ENDENF', ENDENF,'DENF',DENF
      WRITE(TEST+1,*) 'QQ',QQ,'QBQB',QBQB
      WRITE(TEST+1,*) 'ITERCG',ITERCG
      ENDIF
      IF(TEST.LT.2001.OR.TEST.EQ.7000) THEN
      DO 9 I = 1, KVOL
