...
c$omp parallel
      ...
      if(rank.eq.0 .and. omp_get_thread_num() .eq. 0) then
         write(*,*) 'Num of Threads: ', omp_get_num_threads()
         write(*,*) 'Num of Tasks : ', ranksize
      endif
      flush(6)
c$omp end parallel
...
c      if (ismaster) write(*,*) 'H0, S0 = ', H0, S0
c
c      call flush(6)
...
c     hg has already been summed within su2plaq
      ...
      h=hg+hp+hf
c     write(6,*) isweep,':  hg', hg,'   hp', hp,'   hf', hf,'   h',h
      s=hg+hf
c
      return
      end              
...
c     include common block definition
      ...
      real(kind=realkind) chem1,chem2,ranf
c
      one=(1.0,0.0)
      zi=(0.0,1.0)
c*******************************************************************
...
40    continue
      do 61 mu=1,ndim
      do 61 ind=1,kvol
      u11t(ind,mu)=cmplx(2.*ranf()-1.0,2.*ranf()-1.)
      u12t(ind,mu)=cmplx(2.*ranf()-1.0,2.*ranf()-1.)
61    continue
      call reunitarise
      do 62 mu=1,ndim
      do 62 ind=1,kvol
...
1     format(' Hi from gaussp')
      do 1000 il=1,kvol
1000  ps(2,il)=sqrt(-2.0*log(ranf()))
      do 1001 il=1,kvol
      theta=tpi*ranf()
