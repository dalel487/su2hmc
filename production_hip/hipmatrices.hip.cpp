...
void Transpose_z(Complex *out, const int fast_in, const int fast_out, const dim3 dimGrid, const dim3 dimBlock){
	Complex *holder;
	hipMalloc((void **)&holder,fast_in*fast_out*sizeof(Complex));
	hipMemcpy(holder,out,fast_in*fast_out*sizeof(Complex),hipMemcpyDefault);
	Transpose<<<dimGrid,dimBlock>>>(out,holder,fast_in,fast_out);
	//hipblasCgeam_v2(cublas_handle,HIPBLAS_OP_T,HIPBLAS_OP_N,fast_in,fast_out,(hipComplex *)&alpha,\
	//(hipComplex *)out,fast_out,NULL,(hipComplex *)&beta,fast_out,(hipComplex *)holder,fast_out);
	//hipMemcpy(out,holder,fast_in*fast_out*sizeof(Complex_f),hipMemcpyDefault);
	hipFree(holder);
}
void Transpose_c(Complex_f *out, const int fast_in, const int fast_out, const dim3 dimGrid, const dim3 dimBlock){
	Complex_f *holder;
	hipMalloc((void **)&holder,fast_in*fast_out*sizeof(Complex_f));
	hipMemcpy(holder,out,fast_in*fast_out*sizeof(Complex_f),hipMemcpyDefault);
	Transpose<<<dimGrid,dimBlock>>>(out,holder,fast_in,fast_out);
...
void Transpose_d(double *out, const int fast_in, const int fast_out, const dim3 dimGrid, const dim3 dimBlock){
	double *holder;
	hipMalloc((void **)&holder,fast_in*fast_out*sizeof(double));
	hipMemcpy(holder,out,fast_in*fast_out*sizeof(double),hipMemcpyDefault);
	Transpose<<<dimGrid,dimBlock>>>(out,holder,fast_in,fast_out);
	//hipblasCgeam_v2(cublas_handle,HIPBLAS_OP_T,HIPBLAS_OP_N,fast_in,fast_out,(hipComplex *)&alpha,\
	//(hipComplex *)out,fast_out,NULL,(hipComplex *)&beta,fast_out,(hipComplex *)holder,fast_out);
	//hipMemcpy(out,holder,fast_in*fast_out*sizeof(double),hipMemcpyDefault);
	hipFree(holder);
}
void Transpose_f(float *out, const int fast_in, const int fast_out, const dim3 dimGrid, const dim3 dimBlock){
	float *holder;
	hipMalloc((void **)&holder,fast_in*fast_out*sizeof(float));
	hipMemcpy(holder,out,fast_in*fast_out*sizeof(float),hipMemcpyDefault);
	Transpose<<<dimGrid,dimBlock>>>(out,holder,fast_in,fast_out);
	//hipblasCgeam_v2(cublas_handle,HIPBLAS_OP_T,HIPBLAS_OP_N,fast_in,fast_out,(hipComplex *)&alpha,\
	//(hipComplex *)out,fast_out,NULL,(hipComplex *)&beta,fast_out,(hipComplex *)holder,fast_out);
	//hipMemcpy(out,holder,fast_in*fast_out*sizeof(float),hipMemcpyDefault);
	hipFree(holder);
}
void Transpose_I(int *out, const int fast_in, const int fast_out, const dim3 dimGrid, const dim3 dimBlock){
	int *holder;
	hipMalloc((void **)&holder,fast_in*fast_out*sizeof(int));
	hipMemcpy(holder,out,fast_in*fast_out*sizeof(int),hipMemcpyDefault);
	Transpose<<<dimGrid,dimBlock>>>(out,holder,fast_in,fast_out);
	//hipblasCgeam_v2(cublas_handle,HIPBLAS_OP_T,HIPBLAS_OP_N,fast_in,fast_out,(hipComplex *)&alpha,\
	//(hipComplex *)out,fast_out,NULL,(hipComplex *)&beta,fast_out,(hipComplex *)holder,fast_out);
	//hipMemcpy(out,holder,fast_in*fast_out*sizeof(int),hipMemcpyDefault);
	hipFree(holder);
}
void Transpose_U(unsigned int *out, const int fast_in, const int fast_out, const dim3 dimGrid, const dim3 dimBlock){
	unsigned int *holder;
	hipMalloc((void **)&holder,fast_in*fast_out*sizeof(unsigned int));
	hipMemcpy(holder,out,fast_in*fast_out*sizeof(unsigned int),hipMemcpyDefault);
	Transpose<<<dimGrid,dimBlock>>>(out,holder,fast_in,fast_out);
	//hipblasCgeam_v2(cublas_handle,HIPBLAS_OP_T,HIPBLAS_OP_N,fast_in,fast_out,(hipComplex *)&alpha,\
	//(hipComplex *)out,fast_out,NULL,(hipComplex *)&beta,fast_out,(hipComplex *)holder,fast_out);
	//hipMemcpy(out,holder,fast_in*fast_out*sizeof(int),hipMemcpyDefault);
