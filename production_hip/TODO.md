...
3.  HIP is clang based so a lot stricter about certain coding practices. For example there were '#pragma omp simd'
...
results. We suspect this is due to the lack of xnack on the testbed GPU but until we run on officially supported ones we
cannot be certain.
