...
void	Init_CUDA(Complex *u11t, Complex *u12t,Complex *gamval, Complex_f *gamval_f, int *gamin, double*dk4m,\
	...
	hipMemPrefetchAsync(iu,ndim*kvol*sizeof(int),device,streams[0]);
	hipMemPrefetchAsync(id,ndim*kvol*sizeof(int),device,streams[1]);
	hipMemAdvise(iu,ndim*kvol*sizeof(int),hipMemAdviseSetReadMostly,device);
	hipMemAdvise(id,ndim*kvol*sizeof(int),hipMemAdviseSetReadMostly,device);

	//Gamma matrices and indices on the GPU
	//	hipMemcpy(gamin_d,gamin,4*4*sizeof(int),hipMemcpyHostToDevice);
	//	hipMemcpy(gamval_d,gamval,5*4*sizeof(Complex),hipMemcpyHostToDevice);
	//	hipMemcpy(gamval_f_d,gamval_f,5*4*sizeof(Complex_f),hipMemcpyHostToDevice);
	hipMemAdvise(gamin,4*4*sizeof(int),hipMemAdviseSetReadMostly,device);
	hipMemAdvise(gamval,5*4*sizeof(Complex),hipMemAdviseSetReadMostly,device);

	//More prefetching and marking as read-only (mostly)
	//Prefetching Momentum Fields and Trial Fields to GPU
	hipMemAdvise(dk4p,(kvol+halo)*sizeof(double),hipMemAdviseSetReadMostly,device);
	hipMemAdvise(dk4m,(kvol+halo)*sizeof(double),hipMemAdviseSetReadMostly,device);

	hipMemPrefetchAsync(dk4p,(kvol+halo)*sizeof(double),device,streams[2]);
	hipMemPrefetchAsync(dk4m,(kvol+halo)*sizeof(double),device,streams[3]);

	hipMemPrefetchAsync(u11t, ndim*kvol*sizeof(Complex),device,streams[4]);
	hipMemPrefetchAsync(u12t, ndim*kvol*sizeof(Complex),device,streams[5]);
}
