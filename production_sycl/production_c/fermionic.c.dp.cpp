...
#include	<random.h>
#include	<su2hmc.h>
int Measure(double *pbp, double *endenf, double *denf, Complex *qq,
            Complex *qbqb, double res, int *itercg, Complex *u11t,
            Complex *u12t, Complex_f *u11t_f, Complex_f *u12t_f,
            unsigned int *iu, unsigned int *id, Complex *gamval,
            Complex_f *gamval_f, int *gamin, double *dk4m, double *dk4p,
            float *dk4m_f, float *dk4p_f, Complex_f jqq, float akappa,
 ...
 sycl::queue &q_ct1 = dev_ct1.in_order_queue();
	...
	cudaMallocAsync((void **)&R1_f,kfermHalo*sizeof(Complex_f),streams[1]);
        cudaMallocManaged((void **)&x, kfermHalo * sizeof(Complex), 0);
        cudaMallocManaged((void **)&xi, kferm * sizeof(Complex), 0);
        cudaMallocManaged((void **)&xi_f, kfermHalo * sizeof(Complex_f), 0);
#else
...
*/
        ...
        cudaFreeAsync(R1_f, streams[1]); dpct::dpct_free(xi_f, q_ct1);
#else
...
#ifdef DPCT_COMPATIBILITY_TEMP
        dpct::dpct_free(x, q_ct1); cudaFree(xi);
#else
	free(x); free(xi);
#endif
	return 0;
