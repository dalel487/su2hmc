 ...
 * @file congrad.c
 *
 * @brief Conjugate gradients. Congradq for the solver and Congradp for the
 * inversion
 */
#include	<matrices.h>
...
int Congradp(int na,double res,Complex *Phi,Complex *xi,Complex_f *u11t,Complex_f *u12t,unsigned int *iu,unsigned int *id,\
	...
	//Give initial values Will be overwritten if niterx>0
        ...
        int device; device = dpct::dev_mgr::instance().current_device_id();
#ifdef _DEBUG
...
#endif
#ifdef DPCT_COMPATIBILITY_TEMP
        dpct::dpct_free(p_f, dpct::get_in_order_queue()); cudaFree(r_f);
            cudaFree(x1_f); cudaFree(x2_f); cudaFree(xi_f);
#else
