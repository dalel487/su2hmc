 ...
 * @file bosonic.c
 *
 * @brief Code for bosonic observables, Basically polyakov loop and Plaquette
 * routines.
 */
#include	<par_mpi.h>
#include	<su2hmc.h>
...
double Polyakov(Complex_f *u11t, Complex_f *u12t){
	...
	Sigma11=sycl::malloc_shared(kvol3 * sizeof(Complex_f), dpct::get_in_order_queue());
#ifdef _DEBUG
	Sigma12=sycl::malloc_shared(kvol3*sizeof(Complex_f),dpct::get_in_order_queue());
#else
	Sigma12 sycl::malloc_device(kvol3*sizeof(Complex_f),dpct::get_in_order_queue());
#endif
#else
...
#ifdef DPCT_COMPATIBILITY_TEMP
	blas::copy(cublas_handle,kvol3, (Complex_f *)(u11t+3), ndim, (Complex_f *)Sigma11, 1);
	blas::copy(cublas_handle,kvol3, (Complex_f *)(u12t+3), ndim, (Complex_f *)Sigma12, 1);
...
#if (npt>1)
#ifdef DPCT_COMPATIBILITY_TEMP
#error Par_tmul is not yet implimented in SYCL  as Sigma12 is device only memory
#endif
#ifdef _DEBUG
...
#ifdef DPCT_COMPATIBILITY_TEMP
	dpct::dpct_free(Sigma11, dpct::get_in_order_queue());
	dpct::dpct_free(Sigma12, dpct::get_in_order_queue());
#else
